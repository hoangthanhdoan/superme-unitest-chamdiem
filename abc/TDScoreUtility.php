...
class TDScoreUtility
{
    ...
    public function setUsingTime(int $usingTime)
    {
    ...
    /**
     * Calculate number of  miss spelling
     *
     * @param int $r Row index
     * @param array $questions Array of questions
     * @return float|int number off miss spelling on a row 
     */
    public function countMissSpellingOnRow(int $r, array $questions)
    {
        $missSpellingCount = 0;
     ...
     * @return array Result containing score and correct count
     */
    public function scoreWord(array $testData, bool $isReCalculateValueCorrect = false, array $wordReviews = []): array
    {
        $questions = $testData['items'] ?? [];
        $score = 0;
        $correctCount = 0;
        ...
        for ($i = 0; $i < $this->count; $i++) {
            ...
            if ($isReCalculateValueCorrect) {
                ...
                if (isset($re_questionItemDict['value1'], $re_questionItemDict['input1'])) {
                    ...
                    // Nếu sai, kiểm tra trong WordReview
                    if (!$re_questionItemDict['isCorrect1']) {
                        // Tìm WordReview phù hợp cho item hiện tại
                        $matchingWordReview = null;
                        foreach ($wordReviews as $wordReview) {
                            $isCorrectFromAppeal = $this->compareStrings(
                                $wordReview->correct_word,
                                $re_questionItemDict['value1']
                            ) && $this->compareStrings(
                                $wordReview->input_word,
                                $re_questionItemDict['input1']
                            );
                            
                            if ($isCorrectFromAppeal) {
                                $matchingWordReview = $wordReview;
                                break;
                            }
                        }
                        
                        if ($matchingWordReview && $matchingWordReview->final_conclusion == 1) {
                            $re_questionItemDict['isCorrect1'] = true;
                        }
        ...
        for (; $r >= 0; $r--) {
            if (!$this->checkEmptyRow($r, $questions)) {
                $scoreAtRow = $this->scoreForEndRow($r, $questions);
                $missSpellingCount = $this->countMissSpellingOnRow($r, $questions);
                $scoreAtRow -= $missSpellingCount;
                if ($scoreAtRow < 0) {
                    $scoreAtRow = 0;
                }
                $score += $scoreAtRow;
     ...
     * @return array Result containing score and correct count
     */
    public function scoreFace(array $testData, bool $isReCalculateValueCorrect = false, array $wordReviews = []): array
    {
        $questions = $testData['items'] ?? [];
        $score = 0;
        $correctCount = 0;
            ...
            if (true) {
                ...
                if (isset($re_questionItemDict['value2'], $re_questionItemDict['input2']) &&
                    ...
                    if (!$re_questionItemDict['isCorrect2']) {
                        ...
                        if ($matchingWordReview && $matchingWordReview->final_conclusion == 1) {
                            $re_questionItemDict['isCorrect2'] = true;
                        }
                    }
    ...
    public function public_score_name_and_face(array $testData, bool $isReCalculateValueCorrect = false, array $wordReviews = []): array
    {
        $resultScoreArray = $this->scoreFace($testData, $isReCalculateValueCorrect, $wordReviews);
        $championShipScore = 0;
    ...
    //game_random_word
    public function public_score_random_word(array $testData, bool $isReCalculateValueCorrect = false, array $wordReviews = []): array
    {
        $resultScoreArray = $this->scoreWord($testData, $isReCalculateValueCorrect, $wordReviews);
        $championShipScore = 0;
        if ($resultScoreArray['score'] > 0) {
